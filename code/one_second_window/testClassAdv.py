...
class Evaluator(object):
  ...
  def __init__(self, modelType = ""):
    ...
    self.trainFile = ""
    self.metrics = {}
    self.adversarialClassifierModel = None

    self.modelBaseDir = "model"

    self.XCols = ['mean', 'sum', 'std']
    self.label = 'label'
    ...
    for i, date in enumerate(dates):
      try:
        ...
        if not testX.empty:
          ...
          self.prediction = classifier.predict(testX)
          self.evaluateModelART()



          attack = FastGradientMethod(estimator=self.classifier, eps=0.2)
          self.testXadv = attack.generate(x=testX)

          self.prediction = self.classifier.predict(testXadv)
          evaluateModelART()
  ...
  def evaluateModel(self):
    ...
    self.listObj.update(self.metrics)
  ...
  def evaluateModelART(self):
    self.metrics['acc'] = accuracy_score(self.y, self.prediction)
    self.metrics['recall'] = recall_score(self.y, self.prediction, average="micro")
    self.metrics['f1'] = f1_score(self.y, self.prediction, average="micro")
    self.metrics['spec'] = specificity_score(self.y, self.prediction, average="micro")
    self.metrics['mean'] = geometric_mean_score(self.y, self.prediction, average="micro")
    with open("output.json") as fp:
      self.listObj=json.load(fp)
  ...
  def loadData(self, fileName):
    ...
    self.df = pd.read_csv(fileName, low_memory = False, parse_dates=['time'])
